...
def setup_logging():
    """Setup logging to file in reports/logs/ directory."""
    logs_dir = SCRIPT_DIR / "reports" / "logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    
    # Create log filename with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = logs_dir / f"eval_suite_{timestamp}.log"
    ...
    log_file_handle.write(f"DEEPSCRIBE EVALUATION SUITE LOG\n")
...
def cleanup_logging(log_file_handle, log_file, original_stdout, original_stderr):
    """Restore stdout and close log file."""
    sys.stdout = original_stdout
    sys.stderr = original_stderr
    log_file_handle.write("\n" + "="*80 + "\n")
    log_file_handle.write(f"Completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    log_file_handle.write("="*80 + "\n")
    log_file_handle.close()
    print(f"\n📝 Log saved to: {log_file}")


def run_full_eval_suite(limit=30, generate_charts=True, id_list=None, use_processed=False, force=False, enable_logging=True):
    """
    ...
    original_stderr = sys.stderr
    if enable_logging:
        log_file, log_file_handle, original_stdout, original_stderr = setup_logging()
    ...
    try:
        ...
        if enable_logging and log_file_handle:
            cleanup_logging(log_file_handle, log_file, original_stdout, original_stderr)
